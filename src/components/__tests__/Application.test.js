...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    await waitForElement(() => queryByText(appointment, "Lydia Miller-Jones"));
    // 9. Check that the DayListItem with the text "Monday" keeps the spots remaining the same.
    const day = getAllByTestId(container, "day").find((day) =>
      queryByText(day, "Monday")
    );
  ...
  it("shows the delete error when failing to delete an existing appointment", async () => {
    ...
    // 6. Check that the element with the text "Deleting" is displayed.
    expect(getByText(appointment, "Deleting")).toBeInTheDocument();
    // 8. Wait until the error delete message is displayed.
    await waitForElement(() =>
      queryByText(appointment, "Could not delete appointment.")
    );
    // 9. Click the "Close" button on the form.
    fireEvent.click(getByAltText(appointment, "Close"));
    // 10. Check that existing appointment is being displayed.
